...
package com.intridea.presentlyAIRClient.commands
{
	...
	/**
     *
     * Retrieves friends timeline data from the Present.ly server and updates the data model. The view is bound to
     ...
     * 
     */
	public final class FriendsTimelineCommand extends AbstractCommand implements ICommand, IResponder
	{
		
		/**
		 * 
		 * Sets the current service on the data model. Creates a delegate to gather friends timeline statuses
		...
		{
			model.currentService = "friendsTimeline";
			
			var friendsTimelineEvent : FriendsTimelineEvent = event as FriendsTimelineEvent;
			var delegate : HTTPDelegate = new HTTPDelegate( this, model.currentUser, Services.FRIENDS_TIMELINE, ServiceConfigurations.FRIENDS_TIMELINE_URL );
			delegate.callHTTPService();
		}
		
		/**
		 * 
		 * Handles response from server containing friends timeline data. Updates the 
		...
		override public function result( event : Object ) : void
		{
			...
			var message : String = "New friends messages received from server.";
			if ( statuses )
			{
			...
			{
				message = "You have no friends messages at this time.";
			}
		...
		/**
		 * 
		 * Creates an Alert to notify the user that a problem has occured with the request for the friends timeline 
		 * data from the Present.ly server.
		 * 
		 * @param event Contains error information for failed friends timeline requests.
		 * 
		 */
