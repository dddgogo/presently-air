 ...
 *
 */
package com.presently.components
{
    ...
    public class Notifier extends EventDispatcher
    {
        ...
        public function queueNotification(status:Status, stylesheet:StyleSheet):void {
        ...
        public function displayStackedNotifications():void {
            ...
            if (_notifications.length > 0 && !_displayingStacked) {
                ...
                for (var i:int = 0; i < limit; i++) {
                    ...
                    notification.addEventListener(NotificationWindow.NOTIFICATION_CLOSED,
                        onStackedNotificationClosed);
                    notification.addEventListener(LookupEvent.LOOKUP, onNotificationLookup);
                    notification.addEventListener(NotificationClickedEvent.NOTIFICATION_CLICKED,
                ...
                if (_notifications.length > _maxStacked) {
                    ...
                    currentY = (currentY + firstNotification.height) - (_infoNotification.height + 16);
        ...
        private function onNotificationLookup(event:LookupEvent):void {
            // Redispatch the event
            dispatchEvent(event);
        }

        private function onNotificationClicked(event:NotificationClickedEvent):void {
            // Redispatch the event
            dispatchEvent(event);
        }

        private function sortNotifications(a:NotificationWindow, b:NotificationWindow):Number {
            var aId:Number = a.pStatus.id;
            var bId:Number = b.pStatus.id;
